...
samplecount = len(os.listdir(mainpath+'/zeroscans/'))
    ...
    for u in range(rwidth):
        for v in range(rheight):
            ...
            for i in range(count):
                # print('i=',i)
                folder = '/home/samir/Desktop/blender/pycode/100zeroplanes/render'+ str(i)+'/'
                unwrap = np.zeros((rheight, rwidth), dtype=np.float64)
                unwrap = np.load(folder+'unwrap.npy')   
                phibase[u,v,i] = unwrap[u,v]
    folder = '/home/samir/Desktop/blender/pycode/100zeroplanes/'
...
def makeDBase():
    print('scanumwrap')
    unw('100zeroplanes', 100)
    makeDDbase(100)
# makeDBase()

def makeunwrap():
    unw('30scans', samplecount)
    # print('depth')
    depth('30scans', 5, 400)
    # print('makeclouds')
    makeclouds('30scans', 5)
